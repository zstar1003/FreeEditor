...
const defaultStyles = {
  ...
  img: 'max-width: 100%; height: auto; border-radius: 8px; margin: 0 auto; display: block; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1), 0 8px 24px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(0, 0, 0, 0.05);',
...
${html.replace(
  ...
  const copyToClipboard = async () => {
    try {
      ...
      // 处理 highlight.js 代码高亮：将 class 样式转换为内联样式
      // 微信公众号不支持外部 CSS，需要将所有 hljs-* 类名转换为内联样式
      const hljsStyles: { [key: string]: string } = {
    ...
    <div className={`preview-panel ${theme} ${!showPreview ? 'preview-hidden' : ''}`}>
      <div className="panel-header">
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          {onXiaohongshuCard && (
            <button
              className="preview-toggle-btn xiaohongshu-btn"
              onClick={onXiaohongshuCard}
              title="小红书卡片"
            >
              <svg width="20" height="20" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
